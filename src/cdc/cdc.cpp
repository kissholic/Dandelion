...
// Copied from https://github.com/google/cdc-file-transfer/blob/main/fastcdc/fastcdc.h
...
static size_t estimate_chunk_num(size_t const length) noexcept {
    return length / (1 << CDC_PATTERN_MASK_BITS);
}
...
std::unique_ptr<std::vector<cdc_chunk>> cdc(char const* data, size_t const length) noexcept {
    ...
    for (std::int64_t offset = 0; offset <= total_file_size - sizeof(int); ++offset) {
        ...
        if ((pattern & CDC_PATTERN_MASK) == CDC_PATTERN_MAGIC) {
